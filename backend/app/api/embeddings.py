...
These endpoints integrate with embedding generation scripts
...
class ScriptOutput(BaseModel):
    """Expected output format from external embedding script."""
    document_id: str = Field(..., description="Document UUID as string")
    embedding: List[float] = Field(..., description="Generated embedding vector")
    model_name: str = Field(default="external-script-v1.0", description="Model identifier")
...
async def store_embedding(
    ...
    Use this endpoint to store embeddings generated by external scripts.
    """
    try:
        ...
        # Matching is now triggered only by scheduled batch jobs, not by API calls.
...
@router.post("/store-from-script", response_model=EmbeddingResponse)
async def store_from_script(
    script_output: ScriptOutput,
    session: Session = Depends(get_session)
):
    """
    Store embedding directly from external script output format.
    
    This endpoint is designed to match the exact output format of
    external embedding generation scripts for easy integration.
    """
    try:
        service = EmbeddingService(session)
...
# CSV Processing endpoints for external workflows
...
async def process_csv_file(
    ...
    """
    Process a CSV file generated by external embedding script.
...
async def process_csv_directory(
    ...
    Useful when external scripts generate multiple CSV files.
...
async def batch_store_embeddings(
    embeddings_data: List[ScriptOutput],
    ...
    Useful when external scripts process many CVs at once.
    """
    try:
        service = EmbeddingService(session)
