...
def upgrade():
    ...
    op.add_column('documents', sa.Column('is_latest', sa.Boolean(), nullable=False, server_default='false'))

    
    op.alter_column('documents', 'language', type_=sa.String(length=2))
    op.create_check_constraint('check_status_valid', 'documents', "status IN ('uploaded', 'parsed', 'failed')")
    ...
    try:
        op.create_foreign_key('fk_embeddings_document', 'embeddings', 'documents', ['document_id'], ['id'], ondelete='CASCADE')
    except:
    ...
    op.add_column('search_results', sa.Column('created_at', sa.TIMESTAMP(), server_default=sa.text('now()')))
    op.alter_column('search_results', 'clicked', server_default=sa.text('false'))
    
    # FK constraints (verifica se non esistono già)
    try:
        op.create_foreign_key('fk_search_results_document', 'search_results', 'documents', ['document_id'], ['id'], ondelete='CASCADE')
    except:
        pass
    
    try:
        op.create_foreign_key('fk_search_results_search', 'search_results', 'searches', ['search_id'], ['id'], ondelete='CASCADE')
    except:
        pass
...
def downgrade():
    ...
    op.drop_constraint('fk_search_results_document', 'search_results', type_='foreignkey')
