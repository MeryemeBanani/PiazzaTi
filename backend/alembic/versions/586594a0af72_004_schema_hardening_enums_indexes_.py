...
def downgrade() -> None:
    ...
    op.execute(text("ALTER TABLE documents DROP CONSTRAINT IF EXISTS check_language_format"))
    op.execute(text("ALTER TABLE embeddings DROP CONSTRAINT IF EXISTS check_vector_normalized"))
    
    # Rimuovi indice performance
    op.execute(text("DROP INDEX IF EXISTS idx_jd_open_lang"))
    
    # Ripristina constraint senza CASCADE
    op.execute(text("ALTER TABLE search_results DROP CONSTRAINT IF EXISTS search_results_search_id_fkey"))
    op.execute(text("""
        ALTER TABLE search_results 
        ADD CONSTRAINT search_results_search_id_fkey 
